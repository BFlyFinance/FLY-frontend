...
enum PROVIDER_URL_MAP {
    ...
    "https://halley-seed.starcoin.org" = 254,
}

if (window.starcoin) {
    starcoinProvider = new providers.Web3Provider(window.starcoin, "any");
    jsonProvider = (url: string) => new providers.JsonRpcProvider(url || PROVIDER_URL_MAP[window.starcoin?.networkVersion || 1]);
} else {
 ...
 * 带类型的数据变换
 */
export const SerizalWithType = (params: any = { value: "", type: {} }) => {
    const { value = "", type = "" } = params;
    if (type === "Address") return arrayify(value);

    const se: any = new bcs.BcsSerializer();
...
const TxnWrapper = async ({
    ...
    try {
        ...
        const { args: functionResolve } = await jsonProvider().send("contract.resolve_function", [functionId]);
        ...
        utils.tx.encodeScriptFunction(functionId, typeTag ? utils.tx.encodeStructTypeTags(Array.isArray(typeTag) ? typeTag : [typeTag]) : [], args).serialize(se);
